...
import { simplifyLine } from '@/lib/text-utils.js'
...
export function getItemClasses(item, device, mode) {
  return item.classes.filter(cls => cls.device === device && cls.mode === mode)
}
...
export function getItemEditorClass(item, device, mode, plugins = importedPlugins ) {
  ...
  const classes = getItemClasses(item, device, mode)
  // console.log('editor classes: ' + classes.length )
  // console.log()
  return simplifyLine(classes.map(cls =>   getEditorClass(cls, plugins)).join(' '))
}
...
export function getItemRenderedClass(item, plugins = importedPlugins) {
  ...
  return simplifyLine(optimized.map(cls =>  getRenderedClass(cls, plugins)).join(' '))
}