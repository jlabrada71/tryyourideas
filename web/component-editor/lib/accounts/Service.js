...
import { ActivationEmailRepository } from '@/lib/accounts/ActivationEmailRepository.js'
...
export function getAccountService(config) {
  ...
  const activationEmailRepository = new ActivationEmailRepository(config)
  return new AccountService(accountRepository, credentialService, emailService, activationEmailRepository, idFromEmail, getRandomId )
}

export class AccountService {
  constructor(accountRepository, credentialService, emailService, activationEmailRepository, idCreator, activationCodeCreator) {
  ...
  newAccount( { name, email, mailing, licence = 'community', maxProjects = 3 }) {
  ...
  async sendActivationEmail(account) {
    const result = `<html>
      ...
      try {
        await this.emailService.send({ title: 'TryYourIdeas Account Activation', email: account.email, content: result })
        this.activationEmailRepository.insert({ title: 'TryYourIdeas Account Activation', email: account.email, content: result })
      }
      catch(e) {
  ...
  async resendActivationMessages() {
    const inactiveAccounts = await this.accountRepository.select(acc => !acc.active)
    inactiveAccounts.forEach(async account => {
      try {
        console.log(`resending activation for account `)
        console.log(account)
        await this.sendActivationEmail(account)
  ...
  async findAndUpdateUserFromGithub({
    ...
    if (githubAccounts.length == 1) {
      ...
      githubAccount.updateDate = Date.now()
    ...
    try {
      ...
      newAccount.creationDate = Date.now()
