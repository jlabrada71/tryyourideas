<template>
  <div id="signUpForm" data-modal-backdrop="static" tabindex="-1" aria-hidden="true" class="fixed top-0 left-0 right-0 z-50 hidden w-full p-4 overflow-x-hidden overflow-y-auto md:inset-0 h-[calc(100%-1rem)] max-h-full">
    <div class="flex flex-row flex-wrap gap-2 justify-center bg-slate-100 w-80 h-auto rounded-lg ">
        <div class=" flex flex-row flex-wrap justify-end bg-inherit w-full h-10 rounded-lg ">
            <button  ref="closeElement" data-modal-hide="signUpForm"  type="button" class=" flex flex-row bg-inherit w-6 h-6 rounded-full hover:flex hover:flex-row hover:bg-slate-300 ">
      ...
      <a :href="getGitHubOAuthUrl(from, githubClientId, '')"
        ...
        <span class=" flex flex-row bg-slate-200 w-auto h-6 text-xs font-semibold ">
            Sign In with Github
        </span>
      </a>
        <!-- <button type="button" class=" flex flex-row gap-3 justify-center items-center mx-6 bg-slate-200 w-10/12 h-10 rounded-lg border-4 border-slate-500 transition-all hover:bg-slate-200  hover:border-slate-500 hover:translate-y-1 ">
          <img :src="GoogleLogo" class="h-8" alt="Google Logo" />
          <span class=" flex flex-row bg-slate-200 w-auto h-6 text-xs font-semibold ">
            Sign In with Google
          </span>
        </button> -->
      <span class=" bg-inherit w-full h-6 text-center ">
          or
      </span>
      <input 
              v-model="form.name" 
