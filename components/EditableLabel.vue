<template>
  <div :class="labelClass" @click.stop="select">
    ...
    <div v-if="!isEditing" class="hidden justify-self-end group-hover:block group-focus:block gap-4">  
...
<script setup>
  ...
  const buttonClass = computed(() => `w-5 hover:${props.bgcolor}-800  dark:${props.bgcolor}-600 dark:hover:${props.bgcolor}-700 font-medium rounded-lg text-sm`)
  ...
  function handleBlur() {
    ...
    // console.log(`validating '${props.text}' using: ${props.validator}`)
    const match = regex.exec(props.text);
    // console.log(match)
    if (!match || match[0] !== props.text) {
