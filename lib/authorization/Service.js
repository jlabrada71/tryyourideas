...
export const getGoogleOauthToken = async ({ code }, config) => {
  ...
  try {
    ...
    log('Failed to fetch Google Oauth Tokens' + err.message);
    throw new Error(err);
  }
}
...
export const getGithubOauthToken = async ({ code }, config) => {
  const rootUrl = 'https://github.com/login/oauth/access_token'
  debug(config.public.githubOAuthClientId)
  debug(config.githubOAuthClientSecret)
  const options = {
  ...
  try {
    ...
    if (decoded.error) {
      debug('decoded error: ' + decoded.error_description)
...
export class AuthorizationService {
  ...
  async signinWithGithubCode(code) {
    ...
    try {
      ...
      const { access_token } = await getGithubOauthToken({ code }, this.config);
      ...
      log('Could not authenticate: ' + err.message)
      throw new AuthorizationError('Could not authenticate' + err.message, { errorCode: '05' }, err)
    }
  }
}
