...
import { generateProperties } from '@/lib/generators/ItemPropertyUtils.js'
...
function generateSvg(item) {
  const code = item.properties.find( prop => prop.name === 'svg').value
  const finalCode = code.replace('" ',`" id="id-${item.id}"  class="${cleanText(getItemRenderedClass(item))}" ` )
  return finalCode
}

export function itemToHtml(item, component) {
  if (!item.type) return ''
  if (item.isComponent) {
    return `<${item.type} id="id-${item.id}" ${generateProperties(item.properties, component)}></${item.type}>`
  }
  if (item.type === 'Icon') return generateSvg(item)
  const code = `<${item.type} id="id-${item.id}" 
                  ${generateProperties(item.properties, component)}
