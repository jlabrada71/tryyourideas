...
function generateMargin(itemClass, f) {
  console.log(`${f(itemClass.marginTop)}  ${f(itemClass.marginLeft)} ${f(itemClass.marginRight)} ${f(itemClass.marginBottom)}`)
...
export function getClassString( itemClass, f ) {
  ...
  return ` ${f(itemClass.display)}
           ...
           ${margin} 
           ${f(itemClass.backgroundColor)} ${f(itemClass.width)} ${f(itemClass.height)} ${f(itemClass.padding)} ${f(itemClass.spacing)}
