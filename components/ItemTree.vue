    ...
    <ul class="flex flex-col pl-3">
      <div :class="itemClass" @click="emit('selected', item)">
        ...
        <div class="hidden group-hover:block group-focus:block">
          <div class="flex flex-row justify-self-end gap-4">            
            <button v-if="!isSelfClosing" type="button" class="text-white w-5 hover:text-white font-medium rounded-lg text-sm  " @click.stop="emit('update:add-child', item )">
            ...
            <button type="button" class="text-white w-5 hover:text-white font-medium rounded-lg text-sm"  @click.stop="emit('update:remove', item )">
