...
export class CredentialService {
  ...
  async isValid( { email, password }) {
    const credentials = await this.credentialService.getCredentialsByEmail(email)
    ...
    const isCorrect = isPasswordCorrect(credential.password.hash, credentials.password.salt, credentials.password.iterations, `${email}::${password}`)
    return isCorrect
  }
}